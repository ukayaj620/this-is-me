...
const ProjectCard = ({ title, image, associated, tech, team, description, links }) => {
  return (
    <div className="w-full my-3 lg:px-8 py-2 flex flex-col lg:flex-row items-center">
      <img
        className="w-36 h-36 my-2 lg:mr-12"
        src={images(image)}
        alt={title}
      />
      <div className="flex flex-col justify-center items-center lg:items-start">
        <h3>{title}</h3>
        <div className="flex flex-col font-semibold mt-0.5 mb-2 items-center lg:items-start">
          ...
          {team && <h5 className="text-gray-500 font-medium">Team: {team}</h5>}
        </div>
